...
async function handleApi(req, res, url) {
  ...
  if (m === 'POST' && p === '/api/vardana/production') {
    ...
    await updateStore((store) => {
      ...
      store.products[product].available = Number(store.products[product].available || 0) + qtyMade;
  ...
  if (vardanaReverseMatch && m === 'POST') {
    ...
    await updateStore((store) => {
      ...
      if (store.products[h.product]) {
        store.products[h.product].available = Math.max(0, Number(store.products[h.product].available || 0) - Number(h.qty || 0));
