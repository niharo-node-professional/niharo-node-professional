...
function renderAll() {
  updateAuthUi(); renderDropdowns(); renderKpis(); renderCart(); updateOrderConversionFields(); updateStockConversionFields(); updateClosingBalancePreview(); renderRecentOrders(); renderInventory(); renderInventoryHistory(); renderOrders(); renderMasters(); renderTargets(); renderReports(); renderVardana(); renderAdminPanel(); applyPermissions();
...
function productAltUnit(productName) { const u = String(state.products?.[productName]?.altUnit || '').trim().toUpperCase(); return PRODUCT_UNITS.includes(u) ? u : ''; }
function productConv(productName) { const n = Number(state.products?.[productName]?.conversion || state.products?.[productName]?.conversionRate || 0); return n > 0 ? n : 0; }
function hasConversion(productName) { return !!(productAltUnit(productName) && productConv(productName) > 0 && productAltUnit(productName) !== productUnit(productName)); }
function altToBase(productName, altQty) { const c = productConv(productName); return c > 0 ? Number(altQty || 0) * c : Number(altQty || 0); }
function baseToAlt(productName, baseQty) { const c = productConv(productName); return c > 0 ? Number(baseQty || 0) / c : Number(baseQty || 0); }
function conversionText(productName) { return hasConversion(productName) ? `1 ${productAltUnit(productName)} = ${qty(productConv(productName))} ${productUnit(productName)}` : 'No alternate unit'; }
...
function renderInventory() {
  ...
  const rows = items.map(p => {
    ...
    const altUnit = productAltUnit(p);
...
window.updateProductConversion = async (encoded, altUnit, conversion) => { if (!requirePerm('inventory','edit')) { renderInventory(); return; } const name = decodeURIComponent(encoded); const product = state.products[name]; if (!product) return; try { const data = await api('/api/products/' + encodeURIComponent(name), { method: 'PUT', body: { location: product.location || 'MAIN', available: product.available || 0, unit: product.unit || 'PCS', altUnit, conversion } }); state = normalizeState(data.state || data.data); renderAll(); toast('Conversion updated', 'success'); } catch(e) { toast(e.message, 'error'); } };
...
function groupWorkerTotalsByCategory(rows) {
  const map = {};
  rows.forEach(r => {
    const category = r.category || productCategory(r.product);
    const unit = r.unit || productUnit(r.product);
    const key = category + '||' + r.product + '||' + unit;
    if (!map[key]) map[key] = {
      category,
      product: r.product,
      unit,
    ...
    const qtyPending = Number(r.pending || r.short || 0);
    map[key].pending += qtyPending;
    const partyKey = r.party || '-';
    map[key].parties[partyKey] = (map[key].parties[partyKey] || 0) + qtyPending;
    if (r.salesman) map[key].salesmen.add(String(r.salesman).toUpperCase());
    if (r.priority) map[key].priorities.add(String(r.priority));
  });
  const grouped = {};
  Object.values(map).forEach(item => {
    if (!grouped[item.category]) grouped[item.category] = [];
    const partyBreakup = Object.keys(item.parties).sort((a,b)=>a.localeCompare(b)).map(party => `${party}: ${qty(item.parties[party])} ${item.unit}`).join(' | ');
    grouped[item.category].push({
      category: item.category,
      product: item.product,
      unit: item.unit,
      pending: item.pending,
      parties: partyBreakup,
      salesmen: Array.from(item.salesmen).sort().join(', '),
      priorities: Array.from(item.priorities).sort().join(', ')
    });
  });
...
function downloadProductFormat() {
  const rows = productKeys().map(p => [p, state.products[p]?.location || 'MAIN', Number(state.products[p]?.available || 0), state.products[p]?.unit || 'PCS', state.products[p]?.altUnit || '', state.products[p]?.conversion || '']);
  if (!rows.length) rows.push(['D100','SPICES','100','KG','PCS','0.1']);
  const html = htmlExcelTable('NIHARO PRODUCT MASTER FORMAT', ['Product Name','Category','Available Stock','Base Unit','Alt Unit','1 Alt Unit = Base Qty'], rows);
  downloadTextFile('niharo-product-master-format.xls', html, 'application/vnd.ms-excel');
  toast('Product Excel format downloaded with conversion columns', 'success');
}
...
function resolveProductInput(value) {
  const text = String(value || '').trim();
  if (!text) return '';
  const keys = productKeys();
  const exact = keys.find(p => p.toLowerCase() === text.toLowerCase());
  if (exact) return exact;
  const starts = keys.filter(p => p.toLowerCase().startsWith(text.toLowerCase()));
...
let closingSuggestionIndex = -1;
function highlightClosingSuggestion(index) {
  const box = $('closingBalanceProductSuggest');
  if (!box || box.style.display === 'none') return;
  const items = Array.from(box.querySelectorAll('.product-suggestion-item'));
  if (!items.length) { closingSuggestionIndex = -1; return; }
  closingSuggestionIndex = Math.max(0, Math.min(index, items.length - 1));
  items.forEach((btn, i) => {
    btn.classList.toggle('active', i === closingSuggestionIndex);
    if (i === closingSuggestionIndex) btn.scrollIntoView({ block: 'nearest' });
  });
}
function pickHighlightedClosingSuggestion() {
  const box = $('closingBalanceProductSuggest');
  if (!box || box.style.display === 'none') return false;
  const items = Array.from(box.querySelectorAll('.product-suggestion-item'));
  if (!items.length) return false;
  const idx = closingSuggestionIndex >= 0 ? closingSuggestionIndex : 0;
  const btn = items[Math.max(0, Math.min(idx, items.length - 1))];
  selectClosingProduct(btn.dataset.product || '');
  return true;
}
...
function updateOrderConversionFields() {
  const product = $('orderProduct') ? $('orderProduct').value : '';
  if (!$('orderQty')) return;
  const unit = productUnit(product);
  const altUnit = productAltUnit(product);
  const has = product && hasConversion(product);
  $('orderQtyLabel').textContent = has ? `Qty (${unit})` : `Qty (${unit || 'QTY'})`;
  if ($('orderAltQtyWrap')) $('orderAltQtyWrap').style.display = has ? '' : 'none';
  if (has && $('orderAltQtyLabel')) $('orderAltQtyLabel').textContent = `${altUnit}`;
}
function syncOrderAltFromBase() { const p = $('orderProduct').value; if (!hasConversion(p) || !document.activeElement || document.activeElement.id !== 'orderQty') return; const base = Number.parseFloat($('orderQty').value || '0') || 0; $('orderAltQty').value = base > 0 ? round2(baseToAlt(p, base)) : ''; }
function syncOrderBaseFromAlt() { const p = $('orderProduct').value; if (!hasConversion(p) || !document.activeElement || document.activeElement.id !== 'orderAltQty') return; const alt = Number.parseFloat($('orderAltQty').value || '0') || 0; $('orderQty').value = alt > 0 ? round2(altToBase(p, alt)) : ''; }

function updateStockConversionFields() {
  if (!$('stockName') || !$('stockQty')) return;
  const product = String($('stockName').value || '').trim().toUpperCase().replace(/\s+/g, ' ');
  const unit = productUnit(product);
  const altUnit = productAltUnit(product);
  const has = product && hasConversion(product);
  if ($('stockQtyLabel')) $('stockQtyLabel').textContent = has ? `Qty (${unit})` : `Qty (${unit || 'QTY'})`;
  if ($('stockAltQtyWrap')) $('stockAltQtyWrap').style.display = has ? '' : 'none';
  if ($('stockAltQtyLabel')) $('stockAltQtyLabel').textContent = has ? `Alt Qty (${altUnit})` : 'Alt Qty';
  if ($('stockConversionNoteWrap')) $('stockConversionNoteWrap').style.display = has ? '' : 'none';
  if ($('stockConversionNote')) $('stockConversionNote').textContent = has ? conversionText(product) : 'No conversion';
  if (!has && $('stockAltQty')) $('stockAltQty').value = '';
}
function syncStockAltFromBase() {
  const p = String($('stockName').value || '').trim().toUpperCase().replace(/\s+/g, ' ');
  if (!hasConversion(p) || !document.activeElement || document.activeElement.id !== 'stockQty') return;
  const base = Number.parseFloat($('stockQty').value || '0') || 0;
  $('stockAltQty').value = base > 0 ? round2(baseToAlt(p, base)) : '';
}
function syncStockBaseFromAlt() {
  const p = String($('stockName').value || '').trim().toUpperCase().replace(/\s+/g, ' ');
  if (!hasConversion(p) || !document.activeElement || document.activeElement.id !== 'stockAltQty') return;
  const alt = Number.parseFloat($('stockAltQty').value || '0') || 0;
  $('stockQty').value = alt > 0 ? round2(altToBase(p, alt)) : '';
}
...
function initEvents() {
  ...
  $('drawerBtn').addEventListener('click', () => document.body.classList.toggle('nav-open')); $('globalSearch').addEventListener('input', renderAll); ['filterSalesman','filterStatus','filterShortage','inventoryCategoryFilter','inventoryStockFilter','inventorySortFilter','inventoryHistoryProduct','inventoryHistoryFrom','inventoryHistoryTo','reportFrom','reportTo','reportSalesman','reportParty','reportProduct','targetSalesmanFilter','targetCategoryFilter','targetProductFilter','vardanaRecipeProduct','vardanaProductionProduct'].forEach(id => { if ($(id)) $(id).addEventListener('change', renderAll); }); if ($('closingBalanceProduct')) { $('closingBalanceProduct').addEventListener('input', () => { showClosingProductSuggestions(); updateClosingBalancePreview(); }); $('closingBalanceProduct').addEventListener('focus', showClosingProductSuggestions); $('closingBalanceProduct').addEventListener('change', updateClosingBalancePreview); $('closingBalanceProduct').addEventListener('keydown', e => {
      ...
      if (e.key === 'Enter') {
        ...
        if (matches.length) { e.preventDefault(); selectClosingProduct(matches[0]); }
      }
      if (e.key === 'Escape') hideClosingProductSuggestions();
    }); } if ($('closingBalanceProductSuggest')) { $('closingBalanceProductSuggest').addEventListener('mousedown', e => { const btn = e.target.closest('.product-suggestion-item'); if (btn) { e.preventDefault(); selectClosingProduct(btn.dataset.product || ''); } }); } document.addEventListener('click', e => { if ($('closingBalanceProduct') && $('closingBalanceProductSuggest') && !e.target.closest('#closingBalanceProduct') && !e.target.closest('#closingBalanceProductSuggest')) hideClosingProductSuggestions(); }); if ($('vardanaProductionQty')) $('vardanaProductionQty').addEventListener('input', renderVardanaPreview); if ($('closingBalanceQty')) $('closingBalanceQty').addEventListener('input', updateClosingBalancePreview); if ($('closingBalanceDate')) $('closingBalanceDate').addEventListener('change', updateClosingBalancePreview); if ($('closingBalanceUnit')) $('closingBalanceUnit').addEventListener('change', updateClosingBalancePreview);
  ...
  if ($('stockAltQty')) $('stockAltQty').addEventListener('input', syncStockBaseFromAlt);
