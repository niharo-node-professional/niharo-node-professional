...
function productAltUnit(value) {
  const u = String(value || '').trim().toUpperCase();
  if (!u) return '';
  if (u === 'LTR') return 'LITER';
  return ['PCS', 'KG', 'BOX', 'BAG', 'LITER', 'CARTON'].includes(u) ? u : '';
}
  ...
  if (sourceProducts && typeof sourceProducts === 'object') {
    for (const [rawKey, rawItem] of Object.entries(sourceProducts)) {
      ...
      out.products[name] = {
        ...
        altUnit: productAltUnit(item.altUnit || item.alt_unit || item.alternateUnit || item['Alt Unit'] || item['Alternate Unit']),
...
function productsFromRows(rows) {
  ...
  for (const r of rows.slice(start)) {
    ...
    out.push({ name: first, location: r[1] || 'MAIN', available: r[2] || 0, unit: r[3] || 'PCS', altUnit: r[4] || '', conversion: r[5] || 0 });
  }
  return out;
}
...
async function handleApi(req, res, url) {
  ...
  if (m === 'POST' && p === '/api/admin/login') {
    ...
    if (inputKey === username(ADMIN_USER) && pass === ADMIN_PASSWORD) {
      ...
      return json(res, 200, { token: makeToken('admin', ADMIN_USER, { adminRole: 'Owner' }), user: { username: ADMIN_USER, role: 'admin', adminRole: 'Owner', permissions } });
    }
    ...
    return json(res, 200, { token: makeToken('admin', inputKey, { adminRole: roleName }), user: { username: inputKey, name: account.name || inputKey, role: 'admin', adminRole: roleName, permissions } });
  }
  ...
  if (m === 'POST' && p === '/api/production/quick-selected') {
    if (!need(req, res, ['admin'])) return;
    const b = await body(req);
    ...
    for (const raw of items) {
      const product = productName(raw.product || raw.name);
      const qtyMade = Math.max(0, Number(raw.qty || raw.quantity || 0));
      if (!product || qtyMade <= 0) continue;
      clean.push({ product, qty: qtyMade, unit: productUnit(raw.unit || 'PCS'), category: productName(raw.category || 'MAIN') || 'MAIN' });
    }
    if (!clean.length) return bad(res, 'Valid production qty nahi mili');
    await updateStore((store) => {
      const t = today();
      ...
      for (const row of clean) {
        if (!store.products[row.product]) store.products[row.product] = { name: row.product, location: row.category || 'MAIN', unit: row.unit || 'PCS', available: 0, lastStatus: '', updatedAt: t.iso };
        const opening = Number(store.products[row.product].available || 0);
        const closing = opening + Number(row.qty || 0);
        store.products[row.product].location = store.products[row.product].location || row.category || 'MAIN';
        store.products[row.product].unit = productUnit(store.products[row.product].unit || row.unit || 'PCS');
        store.products[row.product].available = closing;
  ...
  if (m === 'POST' && p === '/api/production/quick-reverse') {
    ...
    await updateStore((store) => {
      const t = today();
      if (!Array.isArray(store.quickProductions)) store.quickProductions = [];
      const entry = store.quickProductions.find(x => String(x.id || '') === id);
      if (!entry) { reverseError = 'Quick production entry nahi mili'; return; }
  ...
  if (m === 'POST' && p === '/api/products/import-file') {
    ...
    await updateStore((store) => {
      for (const raw of products) {
        ...
        const unit = productUnit(raw.unit || raw.Unit || raw.UOM || raw['Unit'] || raw['UOM'] || raw['Base Unit'] || (store.products[name] && store.products[name].unit) || 'PCS');
  ...
  if (productMatch) {
    ...
    if (m === 'PUT') {
      ...
      await updateStore((store) => {
        ...
        store.products[name].altUnit = productAltUnit(b.altUnit || b.alternateUnit || '');
        store.products[name].conversion = conversionValue(b.conversion || b.conversionRate || 0);
