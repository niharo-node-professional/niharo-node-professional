...
function startPolling() { loadState().catch(() => setLive(false)); setInterval(() => loadState().catch(() => setLive(false)), 4000); }
...
function renderDropdowns() {
  ...
  if ($('productSuggestions')) $('productSuggestions').innerHTML = productKeys().map(p => `<option value="${escapeHtml(p)}">${escapeHtml(productCategory(p))} | Stock ${Number(state.products[p]?.available || 0)}</option>`).join('');
...
function renderInventory() { const rows = productKeys().filter(p => filteredText(p, state.products[p].location)).map(p => { const prod = state.products[p]; const demand = productDemand(p); const short = productShortage(p); return `<tr><td><b>${escapeHtml(p)}</b><br><span style="color:var(--muted);font-size:12px">${escapeHtml(prod.lastStatus || '')}</span></td><td><span class="badge badge-blue">${escapeHtml(productCategory(p))}</span></td><td><b style="font-size:17px;color:var(--brand)">${prod.available} pcs</b></td><td>${demand} pcs</td><td>${short ? `<span class="badge badge-danger">${short} pcs</span>` : '<span class="badge badge-success">OK</span>'}</td><td><div class="table-actions"><button class="btn btn-soft btn-sm" onclick="editProduct('${encodeURIComponent(p)}')">Edit</button><button class="btn btn-danger btn-sm" onclick="deleteProduct('${encodeURIComponent(p)}')">Delete</button></div></td></tr>`; }).join(''); $('inventoryBody').innerHTML = rows || '<tr><td colspan="6" class="empty-state">No products found</td></tr>'; }
window.editProduct = async (encoded) => { if (!requireAdmin()) return; const name = decodeURIComponent(encoded); const product = state.products[name]; const location = prompt('Category', product.location || 'MAIN'); if (location === null) return; const available = prompt('Available stock', product.available); if (available === null) return; try { const data = await api('/api/products/' + encodeURIComponent(name), { method: 'PUT', body: { location, available } }); state = normalizeState(data.state || data.data); renderAll(); toast('Product updated', 'success'); } catch (e) { toast(e.message, 'error'); } };
...
function renderOrders() { const filterSm = $('filterSalesman').value || 'ALL'; const filterStatus = $('filterStatus').value || 'Pending'; const filterShort = $('filterShortage').value || 'ALL'; const rows = (state.orders || []).filter(o => { if (filterSm !== 'ALL' && o.salesman !== filterSm) return false; if (filterStatus === 'Pending' && !isPending(o)) return false; if (filterStatus === 'Delivered' && !isDelivered(o)) return false; const hasShort = (o.items || []).some(i => Number(state.products?.[i.product]?.available || 0) < Number(i.qty || 0)); if (filterShort === 'SHORT' && !hasShort) return false; if (filterShort === 'OK' && hasShort) return false; return filteredText(o.party, o.salesman, (o.items || []).map(i => i.product).join(' ')); }).map(o => `<tr><td><b>${escapeHtml(o.party)}</b><br><span style="color:var(--muted);font-size:12px">${escapeHtml(o.date)} ${escapeHtml(o.time || '')}</span></td><td><span class="badge badge-user">${escapeHtml(String(o.salesman || '').toUpperCase())}</span></td><td>${orderItemsHtml(o, true)}</td><td><b>${money(orderTotal(o))}</b></td><td><div class="table-actions">${isPending(o) ? `<button class="btn btn-success btn-sm" onclick="deliverOrder('${o.id}')">Deliver</button><button class="btn btn-blue btn-sm" onclick="downloadPendingOrderJpeg('${o.id}')">JPEG</button>` : '<span class="badge badge-delivered">Delivered</span>'}<button class="btn btn-danger btn-sm" onclick="deleteOrder('${o.id}')">Delete</button></div></td></tr>`).join(''); $('ordersBody').innerHTML = rows || '<tr><td colspan="5" class="empty-state">No orders found</td></tr>'; }
...
window.downloadPendingOrderJpeg = (orderId) => {
  ...
  items.forEach((item, idx) => {
    ...
    ctx.fillText(String(item.qty || 0) + ' pcs', tableX + 610, cy + 36);
...
function renderReports() {
  ...
  $('reportBody').innerHTML = filtered.map(o => `<tr><td>${escapeHtml(o.date)} ${escapeHtml(o.time || '')}</td><td><span class="badge badge-user">${escapeHtml(String(o.salesman || '').toUpperCase())}</span></td><td><b>${escapeHtml(o.party)}</b></td><td>${orderItemsHtml(o, false)}</td><td><b>${money(orderTotal(o))}</b></td><td>${isDelivered(o) ? '<span class="badge badge-delivered">Delivered</span>' : '<span class="badge badge-pending">Pending</span>'}</td></tr>`).join('') || '<tr><td colspan="6" class="empty-state">No report data</td></tr>';
}
...
function downloadProductFormat() {
  const rows = productKeys().map(p => [p, state.products[p]?.location || 'MAIN', Number(state.products[p]?.available || 0)]);
  if (!rows.length) rows.push(['OIL 700','OIL','100']);
  const html = htmlExcelTable('NIHARO PRODUCT MASTER FORMAT', ['Product Name','Category','Available Stock'], rows);
  downloadTextFile('niharo-product-master-format.xls', html, 'application/vnd.ms-excel');
  toast('Product Excel format downloaded with Product, Category, Stock columns', 'success');
}
...
function initEvents() {
  ...
  $('orderItemForm').addEventListener('submit', e => { e.preventDefault(); const product = $('orderProduct').value; const qty = Number.parseInt($('orderQty').value || '0', 10) || 0; const rate = Number.parseFloat($('orderRate').value || '0') || 0; if (!product || qty <= 0) return toast('Product aur quantity required', 'error'); const existing = cart.find(i => i.product === product); if (existing) { existing.qty += qty; existing.rate = rate; } else cart.push({ product, qty, rate }); $('orderQty').value = ''; $('orderRate').value = ''; renderCart(); });
  $('submitOfficeOrderBtn').addEventListener('click', async () => { if (!requireAdmin()) return; const party = $('orderParty').value; if (!party || !cart.length) return toast('Party aur cart items required', 'error'); try { const data = await api('/api/orders', { method: 'POST', body: { party, items: cart, salesman: 'OFFICE ADMIN' } }); state = normalizeState(data.state || data.data); cart = []; renderAll(); toast('Order submitted', 'success'); } catch(e) { toast(e.message, 'error'); } });
