...
function applyActionPermissions() {
  [
    ...
    ['openParserBtn','orders','edit'], ['orderParty','orders','edit'], ['orderProduct','orders','edit'], ['orderQty','orders','edit'], ['orderRate','orders','edit'], ['submitOfficeOrderBtn','orders','edit'],
...
async function api(path, options = {}) {
  ...
  const res = await fetch(path, { ...options, credentials: 'same-origin', headers, body: options.body && typeof options.body !== 'string' ? JSON.stringify(options.body) : options.body });
...
async function loadState() {
  ...
  try {
    ...
    } else if (/no view permission|forbidden|403/i.test(e.message || '')) {
      adminToken = ''; adminUser = ''; adminRole = ''; adminPermissions = {};
      localStorage.removeItem('niharo_admin_token'); localStorage.removeItem('niharo_admin_user'); localStorage.removeItem('niharo_admin_role'); localStorage.removeItem('niharo_admin_permissions');
      updateAuthLock(); renderAll(); showLogin(true);
      toast('Permission/session issue. Dobara login karein.', 'error');
    } else {
      setLive(false);
    }
  }
...
function orderCompactHtml(order, allocation = null) {
  ...
  const shortText = rec ? (rec.ready ? '<span class="badge badge-success">Ready</span>' : `<span class="badge ${rec.shortItems <= 1 ? 'badge-pending' : 'badge-danger'}">${rec.shortItems} short</span>`) : '<span class="badge badge-user">Pending</span>';
  const score = rec ? ` · ${Math.round((rec.pct || 0) * 100)}% allocated` : '';
...
function renderOrders() {
  ...
  const visibleIds = new Set(filteredOrders.map(o => String(o.id)));
  [...dispatchSelectedOrderIds].forEach(id => { if (!visibleIds.has(String(id))) dispatchSelectedOrderIds.delete(id); });
  const rows = filteredOrders.map(o => {
    const rec = allocation[o.id];
    const expanded = expandedOrders.has(String(o.id));
    const checked = dispatchSelectedOrderIds.has(String(o.id)) ? 'checked' : '';
    const selectCell = isPending(o) ? `<input type="checkbox" class="dispatch-check" ${checked} onchange="toggleDispatchSelection('${o.id}', this.checked)" title="Export me include karein">` : '';
    const itemsCell = `<div class="compact-order-cell" onclick="toggleOrderExpand('${o.id}')">${orderCompactHtml(o, rec)}<button class="btn btn-soft btn-sm" type="button">${expanded ? 'Hide items' : 'View items'}</button></div>${expanded ? `<div class="order-expanded">${orderItemsHtml(o, true, rec)}</div>` : ''}`;
    const priority = String(o.priority || 'Normal');
    const prioritySelect = isPending(o) ? `<select class="unit-mini priority-mini" ${permDisabled('orders','edit')} onchange="updateOrderPriority('${o.id}', this.value)"><option ${priority==='High'?'selected':''}>High</option><option ${priority==='Normal'?'selected':''}>Normal</option><option ${priority==='Low'?'selected':''}>Low</option><option ${priority==='Hold'?'selected':''}>Hold</option></select>` : '';
    return `<tr><td class="select-col">${selectCell}</td><td><b>${escapeHtml(o.party)}</b><br><span style="color:var(--muted);font-size:12px">${escapeHtml(o.date)} ${escapeHtml(o.time || '')}</span><br>${prioritySelect}</td><td><span class="badge badge-user">${escapeHtml(String(o.salesman || '').toUpperCase())}</span></td><td>${itemsCell}</td><td><b>${money(orderTotal(o))}</b><br>${isPending(o) ? allocationBadge(rec) : ''}</td><td><div class="table-actions">${isPending(o) ? `<button class="btn btn-warning btn-sm" ${permDisabled('orders','edit')} onclick="startOrderEdit('${o.id}')">Edit</button><button class="btn btn-success btn-sm" ${permDisabled('orders','edit')} onclick="deliverOrder('${o.id}')">Deliver</button><button class="btn btn-blue btn-sm" ${permDisabled('orders','export')} onclick="downloadPendingOrderJpeg('${o.id}')">JPEG</button>` : '<span class="badge badge-delivered">Delivered</span>'}<button class="btn btn-danger btn-sm" ${permDisabled('orders','delete')} onclick="deleteOrder('${o.id}')">Delete</button></div></td></tr>`;
  }).join('');
...
function currentDispatchSelection() {
  ...
  const selectedIds = [...dispatchSelectedOrderIds].filter(id => visibleIds.has(String(id)));
  const exportOrders = selectedIds.length ? orders.filter(o => selectedIds.includes(String(o.id))) : orders;
  return { orders: exportOrders, visibleOrders: orders, allocation, mode, filterSm, filterStatus, filterShort, selectedCount: selectedIds.length };
}
...
function dispatchShortageTotals(sel = currentDispatchSelection()) {
  const map = new Map();
  sel.orders.forEach(o => {
    ...
    (o.items || []).forEach(i => {
      ...
      if (short <= 0) return;
      const unit = productUnit(i.product);
      const key = (i.product || '') + '|' + unit;
      const row = map.get(key) || { product: i.product || '', unit, short: 0, parties: [] };
      row.short += short;
      if (o.party) row.parties.push(o.party);
      map.set(key, row);
    });
  });
  return [...map.values()].sort((a,b) => String(a.product).localeCompare(String(b.product)));
}
function dispatchRowsForExport(sel = currentDispatchSelection()) {
  ...
  sel.orders.forEach((o, orderIndex) => {
    ...
    (o.items || []).forEach(i => {
      const unit = productUnit(i.product);
      const a = rec.items?.[i.product] || { before: '', need: Number(i.qty || 0), allocated: '', short: '', after: '' };
      const short = a.short === '' ? 0 : Number(a.short || 0);
      if (short <= 0) return; // Worker copy me sirf pending/short items niklenge.
      rows.push({
        ...
        before: a.before === '' ? '' : Number(a.before || 0),
        allocated: a.allocated === '' ? '' : Number(a.allocated || 0),
        short,
...
function dispatchWhatsAppText(sel = currentDispatchSelection()) {
  const lines = [];
  lines.push('Niharo Pending Items Plan');
  lines.push(dispatchFilterLabel(sel));
  lines.push('');
  const totals = dispatchShortageTotals(sel);
  if (totals.length) {
    lines.push('TOTAL PENDING ITEMS');
    totals.forEach(t => lines.push(`- ${t.product}: ${qty(t.short)} ${t.unit}`));
    lines.push('');
  }
  let count = 0;
  sel.orders.forEach((o) => {
    const rec = sel.allocation[o.id] || {};
    const shortItems = (o.items || []).filter(i => Number(rec.items?.[i.product]?.short || 0) > 0);
    if (!shortItems.length) return;
    count += 1;
    lines.push(`${count}. ${o.party || ''} (${String(o.salesman || '').toUpperCase()})`);
    lines.push(`Priority: ${o.priority || 'Normal'} | ${o.date || ''} ${o.time || ''}`.trim());
    shortItems.forEach(i => {
      const unit = productUnit(i.product);
      const a = rec.items?.[i.product];
      lines.push(`- ${i.product}: Pending ${qty(a.short)} ${unit} (Demand ${qty(i.qty)} / Alloc ${qty(a.allocated)})`);
    });
    lines.push('');
  });
  if (!totals.length) lines.push('Selected orders me koi pending/short item nahi hai.');
  return lines.join('\n');
}
...
function exportDispatchXls() {
  ...
  const totals = dispatchShortageTotals(sel);
  const headers = ['Order No','Date/Time','Party','Salesman','Priority','Readiness','Product','Demand','Unit','Allocated','Pending/Short','Rate'];
  const body = rows.map(r => [r.orderNo, r.date, r.party, r.salesman, r.priority, r.readiness, r.product, r.demand, r.unit, r.allocated, r.short, r.rate]);
  const totalBody = totals.map(t => ['', '', 'TOTAL PENDING', '', '', '', t.product, '', t.unit, '', t.short, '']);
  const html = htmlExcelTable('NIHARO PENDING ITEMS PLAN - ' + dispatchFilterLabel(sel), headers, totalBody.concat([[]]).concat(body));
...
function exportDispatchPdf() {
  ...
  const sel = currentDispatchSelection();
  const rows = dispatchRowsForExport(sel);
  if (!rows.length) return toast('Selected filter me print data nahi hai', 'error');
  const win = window.open('', '_blank');
  if (!win) return toast('Popup blocked hai. Browser me popup allow karein.', 'error');
  const totals = dispatchShortageTotals(sel);
  const totalRows = totals.map(t => `<tr><td>${escapeHtml(t.product)}</td><td class="right"><b>${qty(t.short)} ${escapeHtml(t.unit)}</b></td></tr>`).join('');
  const totalsBox = totalRows ? `<h2>Total pending items</h2><table><thead><tr><th>Product</th><th class="right">Total pending</th></tr></thead><tbody>${totalRows}</tbody></table>` : '<p><b>No pending/short item in selected orders.</b></p>';
  const summary = sel.orders.map((o, idx) => {
    const rec = sel.allocation[o.id] || {};
    const shortItems = (o.items || []).filter(i => Number(rec.items?.[i.product]?.short || 0) > 0);
    if (!shortItems.length) return '';
    const readyLabel = isPending(o) ? (rec.ready ? 'READY' : ((rec.shortItems || 0) <= 1 ? 'ALMOST READY' : 'PARTIAL')) : 'DELIVERED';
    const itemLines = shortItems.map(i => {
      const unit = productUnit(i.product); const a = rec.items?.[i.product];
      return `<tr><td>${escapeHtml(i.product)}</td><td class="right">${qty(i.qty)} ${escapeHtml(unit)}</td><td class="right">${a ? qty(a.allocated) + ' ' + escapeHtml(unit) : '-'}</td><td class="right"><b style="color:#b42318">${a ? qty(a.short) + ' ' + escapeHtml(unit) : '-'}</b></td></tr>`;
    }).join('');
    return `<h3>${idx+1}. ${escapeHtml(o.party || '')} <span>${escapeHtml(String(o.salesman || '').toUpperCase())}</span> <small>${escapeHtml(readyLabel)} · Priority ${escapeHtml(o.priority || 'Normal')}</small></h3><table><thead><tr><th>Product</th><th class="right">Demand</th><th class="right">Allocated</th><th class="right">Pending/Short</th></tr></thead><tbody>${itemLines}</tbody></table>`;
  }).join('');
  win.document.write(`<!doctype html><html><head><title>Niharo Dispatch Plan</title><style>body{font-family:Arial,sans-serif;margin:24px;color:#111827}.top{display:flex;justify-content:space-between;border-bottom:3px solid #111827;padding-bottom:12px;margin-bottom:16px}h1{margin:0;font-size:24px}.muted{color:#64748b;font-size:12px}button{padding:10px 16px;margin-bottom:14px}h3{margin:18px 0 8px;font-size:16px}h3 span{background:#eef2ff;border-radius:10px;padding:3px 8px;font-size:12px}h3 small{font-size:12px;color:#64748b;font-weight:normal}table{width:100%;border-collapse:collapse;margin-bottom:12px}th{background:#f1f5f9;text-transform:uppercase;font-size:11px;color:#475569}th,td{border:1px solid #dbe3ee;padding:7px;text-align:left;font-size:12px}.right{text-align:right}@media print{button{display:none} body{margin:14px}}</style></head><body><button onclick="window.print()">Print / Save PDF</button><div class="top"><div><h1>NIHARO PENDING ITEMS PLAN</h1><div class="muted">Workers copy / selected orders ke pending items only</div></div><div class="muted">${escapeHtml(new Date().toLocaleString('en-IN'))}<br>${escapeHtml(dispatchFilterLabel(sel))}</div></div>${totalsBox}${summary}<script>setTimeout(()=>window.print(),400)<\/script></body></html>`);
  win.document.close();
}
...
function toggleDispatchSelection(id, checked) {
  if (checked) dispatchSelectedOrderIds.add(String(id)); else dispatchSelectedOrderIds.delete(String(id));
  renderAllocationSummary();
}
function clearDispatchSelection() { dispatchSelectedOrderIds.clear(); renderOrders(); toast('Selection cleared', 'success'); }
function selectShortageOrders() {
  const sel = currentDispatchSelection();
  const alloc = sel.allocation;
  dispatchSelectedOrderIds.clear();
  (sel.visibleOrders || sel.orders || []).forEach(o => { if (isPending(o) && Number(alloc[o.id]?.shortQty || 0) > 0) dispatchSelectedOrderIds.add(String(o.id)); });
  renderOrders();
  toast(dispatchSelectedOrderIds.size + ' shortage orders selected', 'success');
}
window.toggleDispatchSelection = toggleDispatchSelection;
window.clearDispatchSelection = clearDispatchSelection;
window.selectShortageOrders = selectShortageOrders;
...
function initEvents() {
  ...
  if ($('adminUserForm')) $('adminUserForm').addEventListener('submit', saveAdminUser); if ($('rolePermissionRole')) $('rolePermissionRole').addEventListener('change', renderRolePermissions); if ($('saveRolePermissionsBtn')) $('saveRolePermissionsBtn').addEventListener('click', saveRolePermissions); if ($('allocationMode')) { $('allocationMode').value = localStorage.getItem('niharo_allocation_mode') || 'SMART'; $('allocationMode').addEventListener('change', () => { localStorage.setItem('niharo_allocation_mode', $('allocationMode').value); renderRecentOrders(); renderOrders(); }); } if ($('exportDispatchPdfBtn')) $('exportDispatchPdfBtn').addEventListener('click', exportDispatchPdf); if ($('exportDispatchXlsBtn')) $('exportDispatchXlsBtn').addEventListener('click', exportDispatchXls); if ($('copyDispatchWhatsAppBtn')) $('copyDispatchWhatsAppBtn').addEventListener('click', copyDispatchWhatsApp); if ($('openDispatchWhatsAppBtn')) $('openDispatchWhatsAppBtn').addEventListener('click', openDispatchWhatsApp); if ($('selectShortageDispatchBtn')) $('selectShortageDispatchBtn').addEventListener('click', selectShortageOrders); if ($('clearDispatchSelectionBtn')) $('clearDispatchSelectionBtn').addEventListener('click', clearDispatchSelection);
