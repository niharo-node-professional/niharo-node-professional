...
function dispatchShortTotals(rows) {
  const map = {};
  rows.forEach(r => {
    if (!map[r.product]) map[r.product] = { product: r.product, unit: r.unit, pending: 0, orders: 0 };
    map[r.product].pending += Number(r.short || r.pending || 0);
    map[r.product].orders += 1;
  });
  return Object.values(map).sort((a,b) => String(a.product).localeCompare(String(b.product)));
}
...
function renderOrders() {
  ...
  const rows = visibleOrders.map(o => {
    ...
    const selectCell = isPending(o) ? `<input type="checkbox" class="dispatch-check" ${checked} ${disabledCheck} onchange="toggleDispatchSelect('${o.id}', this.checked)" title="Shortage export ke liye select karein">` : '';
    const itemsCell = `<div class="compact-order-cell" onclick="toggleOrderExpand('${o.id}')">${orderCompactHtml(o, rec)}<button class="btn btn-soft btn-sm" type="button">${expanded ? 'Hide items' : 'View items'}</button></div>${expanded ? `<div class="order-expanded">${orderItemsHtml(o, true, rec)}</div>` : ''}`;
...
function dispatchWhatsAppText(sel = currentDispatchSelection()) {
  ...
  if (totals.length) totals.forEach(t => lines.push(`- ${t.product}: ${qty(t.pending)} ${t.unit}`));
  else lines.push('No pending/short items.');
  lines.push('');
...
function exportDispatchXls() {
  ...
  const totals = dispatchShortTotals(rows);
  const summaryHeaders = ['Product','Total Pending','Unit','Orders'];
  const summaryBody = totals.map(t => [t.product, t.pending, t.unit, t.orders]);
  const headers = ['Order No','Date/Time','Party','Salesman','Priority','Product','Demand','Allocated','Pending','Unit'];
  const body = rows.map(r => [r.orderNo, r.date, r.party, r.salesman, r.priority, r.product, r.demand, r.allocated, r.pending, r.unit]);
...
function exportDispatchPdf() {
  ...
  const totals = dispatchShortTotals(rows);
  const totalHtml = totals.map(t => `<tr><td>${escapeHtml(t.product)}</td><td class="right"><b>${qty(t.pending)} ${escapeHtml(t.unit)}</b></td><td class="right">${t.orders}</td></tr>`).join('');
  const summary = sel.orders.map((o, idx) => {
    const rec = sel.allocation[o.id] || {};
    const itemLines = (o.items || []).map(i => {
      const unit = productUnit(i.product); const a = rec.items?.[i.product]; const short = Number(a?.short || 0);
      if (short <= 0) return '';
      return `<tr><td>${escapeHtml(i.product)}</td><td class="right">${qty(i.qty)} ${escapeHtml(unit)}</td><td class="right">${qty(a.allocated || 0)} ${escapeHtml(unit)}</td><td class="right"><b style="color:#b42318">${qty(short)} ${escapeHtml(unit)}</b></td></tr>`;
    }).filter(Boolean).join('');
    if (!itemLines) return '';
    return `<h3>${idx+1}. ${escapeHtml(o.party || '')} <span>${escapeHtml(String(o.salesman || '').toUpperCase())}</span> <small>Priority ${escapeHtml(o.priority || 'Normal')}</small></h3><table><thead><tr><th>Product</th><th class="right">Demand</th><th class="right">Allocated</th><th class="right">Pending</th></tr></thead><tbody>${itemLines}</tbody></table>`;
  }).filter(Boolean).join('');
  win.document.write(`<!doctype html><html><head><title>Niharo Pending Items</title><style>body{font-family:Arial,sans-serif;margin:24px;color:#111827}.top{display:flex;justify-content:space-between;border-bottom:3px solid #111827;padding-bottom:12px;margin-bottom:16px}h1{margin:0;font-size:24px}.muted{color:#64748b;font-size:12px}button{padding:10px 16px;margin-bottom:14px}h2{font-size:18px;margin:16px 0 8px}h3{margin:18px 0 8px;font-size:16px}h3 span{background:#eef2ff;border-radius:10px;padding:3px 8px;font-size:12px}h3 small{font-size:12px;color:#64748b;font-weight:normal}table{width:100%;border-collapse:collapse;margin-bottom:12px}th{background:#f1f5f9;text-transform:uppercase;font-size:11px;color:#475569}th,td{border:1px solid #dbe3ee;padding:7px;text-align:left;font-size:12px}.right{text-align:right}@media print{button{display:none} body{margin:14px}}</style></head><body><button onclick="window.print()">Print / Save PDF</button><div class="top"><div><h1>NIHARO PENDING ITEMS</h1><div class="muted">Workers copy / only shortage pending material</div></div><div class="muted">${escapeHtml(new Date().toLocaleString('en-IN'))}<br>${escapeHtml(dispatchFilterLabel(sel))}</div></div><h2>Total Pending Items</h2><table><thead><tr><th>Product</th><th class="right">Total Pending</th><th class="right">Orders</th></tr></thead><tbody>${totalHtml}</tbody></table><h2>Party Wise Pending</h2>${summary}<script>setTimeout(()=>window.print(),400)<\/script></body></html>`);
  win.document.close();
}
