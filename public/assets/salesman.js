...
function renderAll() { showApp(); renderDropdowns(); updateSalesmanConversionFields(); renderCart(); renderOrders(); renderTargets(); renderStock(); }
...
function showProductSuggestions() {
  ...
  box.innerHTML = rows.length ? rows.map(p => `<button type="button" class="product-suggestion-item" data-product="${escapeHtml(p)}"><span>${escapeHtml(p)}</span><span class="product-suggestion-meta">Bookable ${qty(bookableStock(p))} ${escapeHtml(productUnit(p))}</span></button>`).join('') : '<div class="product-suggestion-empty">No matching product found</div>';
  box.style.display = 'block';
}
...
function whatsappUnitLabel(unit) {
...
function orderPlainText(order) {
  ...
  (order.items || []).forEach(item => {
    ...
    if (!groups[key]) groups[key] = { category, items: [], totalQty: 0, unit };
  ...
  Object.keys(groups).forEach((groupName, groupIndex) => {
    ...
    lines.push(`Total=${cleanWhatsAppQty(group.totalQty)} ${whatsappUnitLabel(group.unit)}`);
...
window.downloadEstimatedChallan = (orderId) => {
  ...
  items.forEach((item, idx) => {
    ...
    ctx.fillText(qty(item.qty) + ' ' + productUnit(item.product), tableX + 610, y + 39);
...
function renderTargets() {
  ...
  box.innerHTML = rows.map(r => {
    ...
    return `<div class="order-card"><div class="head"><div><b>${escapeHtml(r.key)}</b><div style="color:var(--muted);font-size:12px">${escapeHtml(r.type)} target</div></div><span class="badge ${remaining ? 'badge-pending' : 'badge-success'}">${remaining ? unitQtyText(remaining, u) + ' left' : 'Done'}</span></div><div style="height:8px;background:#eef2f7;border-radius:999px;overflow:hidden;margin:12px 0"><div style="height:100%;width:${pct}%;background:#0f6e56"></div></div><div style="display:flex;justify-content:space-between;color:var(--muted);font-size:13px"><span>Completed ${unitQtyText(r.completed, u)}</span><span>Target ${unitQtyText(r.target, u)}</span></div><div style="display:flex;justify-content:space-between;gap:8px;margin-top:10px;align-items:center"><span class="badge badge-user">Incentive ${r.incentive ? money(r.incentive) : '-'}</span><span class="badge ${targetTimeClass(r.deadline)}">${escapeHtml(timeLeftLabel(r.deadline))}</span></div></div>`;
  }).join('') || '<div class="empty-state">Abhi target set nahi hai.</div>';
}

function renderStock() { const q = ($('smStockSearch').value || '').toLowerCase(); $('smStock').innerHTML = productKeys().filter(p=>p.toLowerCase().includes(q)).map(p=>{ const bookable = bookableStock(p); return `<div class="order-card compact-stock-card"><div class="head"><div><b>${escapeHtml(p)}</b></div><span class="badge ${bookable > 0 ? 'badge-success' : 'badge-danger'}">Bookable ${qty(bookable)} ${escapeHtml(productUnit(p))}</span></div></div>`; }).join('') || '<div class="empty-state">No product found</div>'; }

function updateSalesmanConversionFields() {
  const product = ($('smProductSearch')?.value || '').trim();
  const matched = productKeys().find(p => p.toLowerCase() === product.toLowerCase()) || '';
  const unit = productUnit(matched);
  const has = matched && hasConversion(matched);
  if ($('smQtyLabel')) $('smQtyLabel').textContent = has ? `Qty (${unit})` : `Qty (${unit || 'QTY'})`;
  if ($('smAltQtyWrap')) $('smAltQtyWrap').style.display = has ? '' : 'none';
  if (has && $('smAltQtyLabel')) $('smAltQtyLabel').textContent = productAltUnit(matched);
}
function syncSmAltFromBase() { const product = productKeys().find(p => p.toLowerCase() === ($('smProductSearch').value || '').trim().toLowerCase()) || ''; if (!hasConversion(product) || document.activeElement.id !== 'smQty') return; const base = Number.parseFloat($('smQty').value || '0') || 0; $('smAltQty').value = base > 0 ? round2(baseToAlt(product, base)) : ''; }
function syncSmBaseFromAlt() { const product = productKeys().find(p => p.toLowerCase() === ($('smProductSearch').value || '').trim().toLowerCase()) || ''; if (!hasConversion(product) || document.activeElement.id !== 'smAltQty') return; const alt = Number.parseFloat($('smAltQty').value || '0') || 0; $('smQty').value = alt > 0 ? round2(altToBase(product, alt)) : ''; }

function initEvents() {
  $('smLoginBtn').addEventListener('click', async () => { try { const data = await api('/api/salesman/login', { method: 'POST', body: { username: $('smLoginUser').value, password: $('smLoginPass').value } }); token = data.token; user = data.user || { role: 'salesman', username: data.username }; localStorage.setItem('niharo_salesman_token', token); localStorage.setItem('niharo_salesman_user', JSON.stringify(user)); $('smLoginPass').value = ''; toast('Login successful', 'success'); await loadState(); } catch(e) { toast(e.message, 'error'); } });
  ...
  if (productBox) productBox.addEventListener('mousedown', (e) => { const btn = e.target.closest('.product-suggestion-item'); if (!btn) return; e.preventDefault(); selectProductName(btn.dataset.product || btn.textContent); }); if ($('smProductSearch')) $('smProductSearch').addEventListener('input', updateSalesmanConversionFields); if ($('smQty')) $('smQty').addEventListener('input', syncSmAltFromBase); if ($('smAltQty')) $('smAltQty').addEventListener('input', syncSmBaseFromAlt);
  $('smAddItem').addEventListener('click', () => { const typedProduct = ($('smProductSearch').value || '').trim(); const matchedProduct = productKeys().find(p => p.toLowerCase() === typedProduct.toLowerCase()); const product = matchedProduct || typedProduct.toUpperCase(); const qtyValue = Number.parseFloat($('smQty').value || '0') || 0; const rate = Number.parseFloat($('smRate').value || '0') || 0; if (!matchedProduct || qtyValue <= 0) return toast('List me se valid product aur qty select karein', 'error'); const availableNow = bookableStock(product); if (qtyValue > availableNow && !confirm(`Bookable stock ${qty(availableNow)} ${productUnit(product)} hai. Phir bhi order add karein?`)) return; const existing = cart.find(i=>i.product===product); if (existing) { existing.qty += qtyValue; existing.rate = rate; } else cart.push({ product, qty: qtyValue, rate }); $('smProductSearch').value = ''; $('smQty').value = ''; if ($('smAltQty')) $('smAltQty').value = ''; $('smRate').value = ''; updateSalesmanConversionFields(); renderAll(); setTimeout(() => $('smProductSearch').focus(), 50); });
