...
function renderOrders() {
  ...
  const rows = (state.orders || []).filter(o => {
    ...
    const expanded = expandedOrders.has(String(o.id));
    const itemsCell = `<div class="compact-order-cell" onclick="toggleOrderExpand('${o.id}')">${orderCompactHtml(o)}<button class="btn btn-soft btn-sm" type="button">${expanded ? 'Hide items' : 'View items'}</button></div>${expanded ? `<div class="order-expanded">${orderItemsHtml(o, true)}</div>` : ''}`;
    return `<tr><td><b>${escapeHtml(o.party)}</b><br><span style="color:var(--muted);font-size:12px">${escapeHtml(o.date)} ${escapeHtml(o.time || '')}</span></td><td><span class="badge badge-user">${escapeHtml(String(o.salesman || '').toUpperCase())}</span></td><td>${itemsCell}</td><td><b>${money(orderTotal(o))}</b></td><td><div class="table-actions">${isPending(o) ? `<button class="btn btn-warning btn-sm" ${permDisabled('orders','edit')} onclick="startOrderEdit('${o.id}')">Edit</button><button class="btn btn-success btn-sm" ${permDisabled('orders','edit')} onclick="deliverOrder('${o.id}')">Deliver</button><button class="btn btn-blue btn-sm" ${permDisabled('orders','export')} onclick="downloadPendingOrderJpeg('${o.id}')">JPEG</button>` : '<span class="badge badge-delivered">Delivered</span>'}<button class="btn btn-danger btn-sm" ${permDisabled('orders','delete')} onclick="deleteOrder('${o.id}')">Delete</button></div></td></tr>`;
  }).join('');
