...
async function handleApi(req, res, url) {
  ...
  if (m === 'GET' && p === '/api/state') {
    if (!need(req, res, ['admin', 'salesman'])) return;
  ...
  if (m === 'POST' && p === '/api/admin/login') {
    ...
    if (inputUser.toLowerCase() !== String(ADMIN_USER).toLowerCase() || String(b.password || '') !== ADMIN_PASSWORD) {
      return json(res, 403, { error: 'Wrong user ID or password' });
    }
    return json(res, 200, { token: makeToken('admin', ADMIN_USER), user: { username: ADMIN_USER, role: 'admin' } });
  }
