...
function renderAll() {
  updateAuthUi(); renderDropdowns(); renderKpis(); renderCart(); updateOrderConversionFields(); updateStockConversionFields(); renderRecentOrders(); renderInventory(); renderInventoryHistory(); renderOrders(); renderMasters(); renderTargets(); renderReports(); renderVardana(); renderAdminPanel(); applyPermissions();
...
function quickProductionRowsForSelected() {
  ...
  // Important: selected order ko ready karne ke liye sirf us order ka visible short add karna enough nahi hota.
  // Agar us product ka stock priority/FIFO mein upar ke pending orders consume kar rahe hain,
  // to selected order quick production ke baad bhi partial reh sakta hai.
  // Isliye yahan cumulative demand calculate hoti hai: current priority order mein selected order tak
  // jitna stock required hai, uska shortage produce kiya jayega.
  const mode = currentAllocationMode();
  const orders = sortedPendingOrders(mode);
  ...
  return Object.entries(needed)
    ...
    .map(([product, qtyNeeded]) => ({
      ...
      reason: 'selected orders ready under current priority'
    }))
...
async function quickProductionForSelectedOrders() {
  ...
  if (!confirm('Selected parties ko current priority ke हिसाब se ready karne ke liye Quick Production add karna hai?\n\n' + preview + '\n\nNote: Agar selected order se pehle priority wale orders same product consume kar rahe hain, system unko bhi cover karke selected order ready karega.\n\nIske baad selected orders ready/deliver ho सकेंगे.')) return;
  try {
...
function renderQuickProductionHistory() {
  ...
  body.innerHTML = rows.map(h => {
    const items = (h.items || []).map(i => `<div><b>${escapeHtml(i.product)}</b> — ${qty(i.qty)} ${escapeHtml(i.unit || productUnit(i.product))}</div>`).join('') || '-';
    const orders = Array.isArray(h.orderIds) ? `${h.orderIds.length} selected order(s)` : '-';
    ...
    return `<tr><td><b>${escapeHtml(h.date || '')}</b><br><span class="muted">${escapeHtml(h.time || '')}</span></td><td>${items}</td><td>${orders}</td><td>${escapeHtml(h.note || '')}</td><td>${status}</td><td>${action}</td></tr>`;
