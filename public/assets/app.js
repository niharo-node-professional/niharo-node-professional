...
function renderCart() {
  ...
  box.innerHTML = editNote + cart.map((item, idx) => `<div class="cart-item"><div><b>${escapeHtml(item.product)}</b><div style="color:var(--muted);font-size:13px">${item.qty} pcs × ${money(item.rate)}</div></div><div style="text-align:right"><b>${money(item.qty * item.rate)}</b><br><button class="btn btn-danger btn-sm" onclick="removeCartItem(${idx})">Remove</button></div></div>`).join('') + `<div style="text-align:right;margin-top:10px;font-weight:900">Total: ${money(cart.reduce((s,i)=>s+i.qty*i.rate,0))}</div>`;
}
...
function renderRecentOrders() {
  ...
  $('recentPendingBody').innerHTML = rows || '<tr><td colspan="6" class="empty-state">No pending orders</td></tr>';
}
