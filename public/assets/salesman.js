...
function renderDropdowns() {
  ...
  preserveSelect($('smProduct'), ['<option value="">Select product</option>', ...productKeys().map(p=>`<option value="${escapeHtml(p)}">${escapeHtml(p)}</option>`)].join(''));
}
function renderCart() { if (!cart.length) { $('smCart').innerHTML = '<div class="empty-state" style="padding:14px">Cart empty hai.</div>'; return; } $('smCart').innerHTML = cart.map((item, idx)=>`<div class="cart-item"><div><b>${escapeHtml(item.product)}</b><div style="color:var(--muted);font-size:13px">${item.qty} pcs × ${money(item.rate)}</div></div><div style="text-align:right"><b>${money(item.qty*item.rate)}</b><br><button class="btn btn-danger btn-sm" onclick="removeCart(${idx})">Remove</button></div></div>`).join('') + `<div style="text-align:right;margin-top:10px;font-weight:900">Total: ${money(cart.reduce((s,i)=>s+i.qty*i.rate,0))}</div>`; }
window.removeCart = (idx) => { cart.splice(idx,1); renderCart(); };
function itemsHtml(order) { return (order.items||[]).map(i=>`<div class="item-line">• <b>${escapeHtml(i.product)}</b> — ${i.qty} pcs @ ${money(i.rate)}</div>`).join(''); }
...
function initEvents() {
  ...
  $('smAddItem').addEventListener('click', () => { const product = $('smProduct').value; const qty = Number.parseInt($('smQty').value || '0', 10) || 0; const rate = Number.parseFloat($('smRate').value || '0') || 0; if (!product || qty <= 0) return toast('Product aur qty required', 'error'); const availableNow = bookableStock(product); if (qty > availableNow && !confirm(`Bookable stock ${availableNow} pcs hai. Phir bhi order add karein?`)) return; const existing = cart.find(i=>i.product===product); if (existing) { existing.qty += qty; existing.rate = rate; } else cart.push({ product, qty, rate }); $('smProduct').value = ''; $('smQty').value = ''; $('smRate').value = ''; renderAll(); });
  $('smSubmitOrder').addEventListener('click', async () => { const party = ($('smPartySearch').value || '').trim(); const validParty = (state.parties || []).some(p => String(p).toLowerCase() === party.toLowerCase()); if (!party || !cart.length) return toast('Party aur cart required', 'error'); if (!validParty) return toast('List me se valid party select karein', 'error'); try { const data = await api('/api/orders', { method: 'POST', body: { party, items: cart } }); state = normalizeState(data.state || data.data); cart = []; $('smPartySearch').value = ''; $('smProduct').value = ''; $('smQty').value = ''; $('smRate').value = ''; renderAll(); toast('Order submitted live', 'success'); } catch(e) { toast(e.message, 'error'); } });
