...
function orderPlainText(order) {
  const lines = [];
  lines.push('NIHARO ESTIMATED ORDER');
  lines.push('Party: ' + (order.party || '-'));
  lines.push('Salesman: ' + String(order.salesman || user?.username || '').toUpperCase());
  lines.push('Date: ' + (order.date || '-') + (order.time ? ' ' + order.time : ''));
  lines.push('');
  (order.items || []).forEach((item, idx) => {
    const amount = Number(item.qty || 0) * Number(item.rate || 0);
    lines.push(`${idx + 1}. ${item.product} - ${qty(item.qty)} pcs x ${money(item.rate)} = ${money(amount)}`);
  });
  lines.push('');
  lines.push('Total: ' + money(orderTotal(order)));
  lines.push('');
  lines.push('Note: Ye estimated order copy hai. Final delivery/stock warehouse confirmation ke baad hoga.');
  return lines.join('\n');
}
