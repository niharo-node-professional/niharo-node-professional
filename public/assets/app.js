...
function currentDispatchSelection() {
  ...
  return { orders, allocation, mode, filterSm, filterStatus, filterShort };
}
  ...
  sel.orders.forEach((o, orderIndex) => {
    ...
    (o.items || []).forEach(i => {
      ...
      rows.push({
        ...
        short: a.short === '' ? '' : Number(a.short || 0),
...
function dispatchWhatsAppText(sel = currentDispatchSelection()) {
  const lines = [];
  lines.push('Niharo Dispatch Plan');
  lines.push(dispatchFilterLabel(sel));
  lines.push('');
  sel.orders.forEach((o, idx) => {
    const rec = sel.allocation[o.id] || {};
    const readyLabel = isPending(o) ? (rec.ready ? 'READY' : ((rec.shortItems || 0) <= 1 ? 'ALMOST READY' : 'PARTIAL')) : 'DELIVERED';
    lines.push(`${idx + 1}. ${o.party || ''} (${String(o.salesman || '').toUpperCase()}) - ${readyLabel}`);
    lines.push(`Priority: ${o.priority || 'Normal'} | ${o.date || ''} ${o.time || ''}`.trim());
    (o.items || []).forEach(i => {
      const unit = productUnit(i.product);
      const a = rec.items?.[i.product];
      if (a) {
        const status = Number(a.short || 0) > 0 ? `Short ${qty(a.short)} ${unit}` : `Alloc ${qty(a.allocated)} ${unit}`;
        lines.push(`- ${i.product}: ${qty(i.qty)} ${unit} | ${status}`);
      } else {
        lines.push(`- ${i.product}: ${qty(i.qty)} ${unit}`);
      }
    });
    lines.push('');
  });
  if (!sel.orders.length) lines.push('No pending orders in selected filter.');
  return lines.join('\n');
}
...
function exportDispatchXls() {
  ...
  const headers = ['Order No','Date/Time','Party','Salesman','Priority','Readiness','Product','Demand','Unit','Available Before','Allocated','Short','Rate'];
  const body = rows.map(r => [r.orderNo, r.date, r.party, r.salesman, r.priority, r.readiness, r.product, r.demand, r.unit, r.before, r.allocated, r.short, r.rate]);
  const html = htmlExcelTable('NIHARO DISPATCH PLAN - ' + dispatchFilterLabel(sel), headers, body);
...
function exportDispatchPdf() {
  ...
  const summary = sel.orders.map((o, idx) => {
    ...
    const itemLines = (o.items || []).map(i => {
      ...
      return `<tr><td>${escapeHtml(i.product)}</td><td class="right">${qty(i.qty)} ${escapeHtml(unit)}</td><td class="right">${a ? qty(a.before) + ' ' + escapeHtml(unit) : '-'}</td><td class="right">${a ? qty(a.allocated) + ' ' + escapeHtml(unit) : '-'}</td><td class="right">${a && Number(a.short||0)>0 ? '<b style="color:#b42318">' + qty(a.short) + ' ' + escapeHtml(unit) + '</b>' : '-'}</td></tr>`;
    }).join('');
    return `<h3>${idx+1}. ${escapeHtml(o.party || '')} <span>${escapeHtml(String(o.salesman || '').toUpperCase())}</span> <small>${escapeHtml(readyLabel)} · Priority ${escapeHtml(o.priority || 'Normal')}</small></h3><table><thead><tr><th>Product</th><th class="right">Demand</th><th class="right">Before</th><th class="right">Alloc</th><th class="right">Short</th></tr></thead><tbody>${itemLines}</tbody></table>`;
  }).join('');
  win.document.write(`<!doctype html><html><head><title>Niharo Dispatch Plan</title><style>body{font-family:Arial,sans-serif;margin:24px;color:#111827}.top{display:flex;justify-content:space-between;border-bottom:3px solid #111827;padding-bottom:12px;margin-bottom:16px}h1{margin:0;font-size:24px}.muted{color:#64748b;font-size:12px}button{padding:10px 16px;margin-bottom:14px}h3{margin:18px 0 8px;font-size:16px}h3 span{background:#eef2ff;border-radius:10px;padding:3px 8px;font-size:12px}h3 small{font-size:12px;color:#64748b;font-weight:normal}table{width:100%;border-collapse:collapse;margin-bottom:12px}th{background:#f1f5f9;text-transform:uppercase;font-size:11px;color:#475569}th,td{border:1px solid #dbe3ee;padding:7px;text-align:left;font-size:12px}.right{text-align:right}@media print{button{display:none} body{margin:14px}}</style></head><body><button onclick="window.print()">Print / Save PDF</button><div class="top"><div><h1>NIHARO DISPATCH PLAN</h1><div class="muted">Workers copy / pending material plan</div></div><div class="muted">${escapeHtml(new Date().toLocaleString('en-IN'))}<br>${escapeHtml(dispatchFilterLabel(sel))}</div></div>${summary}<script>setTimeout(()=>window.print(),400)<\/script></body></html>`);
  win.document.close();
}
