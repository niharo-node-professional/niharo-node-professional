...
let state = { products: {}, parties: [], orders: [], salesmen: [], vardana: { materials: {}, recipes: {}, productions: [] }, stockLedger: [], meta: {} };
...
async function loadState() {
  if (!adminToken) {
    state = { products: {}, parties: [], orders: [], salesmen: [], vardana: { materials: {}, recipes: {}, productions: [] }, stockLedger: [], meta: {} };
    setLive(true);
    renderAll();
    showLogin(true);
    return;
  ...
  try {
    ...
    if (/login required|unauthorized|401/i.test(e.message || '')) {
      ...
      localStorage.removeItem('niharo_admin_user');
      renderAll();
      showLogin(true);
...
function updateAuthUi() { $('adminLoginBtn').style.display = isAdmin() ? 'none' : ''; $('adminLogoutBtn').style.display = isAdmin() ? '' : 'none'; $('adminLogoutBtn').textContent = isAdmin() ? 'Logout' : 'Logout'; }
function requireAdmin() { if (!isAdmin()) { showLogin(true); toast('Pehle warehouse login karein', 'error'); return false; } return true; }

function renderAll() {
  updateAuthUi(); renderDropdowns(); renderKpis(); renderCart(); renderRecentOrders(); renderInventory(); renderInventoryHistory(); renderOrders(); renderMasters(); renderTargets(); renderReports(); renderVardana();
...
window.toggleOrderExpand = (orderId) => { const key = String(orderId); if (expandedOrders.has(key)) expandedOrders.delete(key); else expandedOrders.add(key); renderRecentOrders(); renderOrders(); renderReports(); renderVardana(); };
...
function hideLogin() { if (!adminToken) return; $('loginModal').classList.remove('show'); $('adminPassword').value = ''; }
...
window.deleteVardanaMaterial = async (encoded) => { if (!requireAdmin()) return; const name = decodeURIComponent(encoded); if (!confirm('Material delete karein? Recipe se bhi remove ho sakta hai.')) return; try { const data = await api('/api/vardana/materials/' + encodeURIComponent(name), { method: 'DELETE' }); state = normalizeState(data.state || data.data); renderAll(); toast('Material deleted', 'success'); } catch(e) { toast(e.message, 'error'); } };
window.loadVardanaRecipe = (encoded) => { const product = decodeURIComponent(encoded); const r = state.vardana.recipes[product] || {}; $('vardanaRecipeProduct').value = product; $('vardanaRecipeOutputQty').value = r.outputQty || 1; $('vardanaRecipeLines').value = recipeLinesText(r); renderDropdowns(); toast('Recipe loaded for edit'); };
window.deleteVardanaRecipe = async (encoded) => { if (!requireAdmin()) return; const product = decodeURIComponent(encoded); if (!confirm('Recipe delete karein?')) return; try { const data = await api('/api/vardana/recipes/' + encodeURIComponent(product), { method: 'DELETE' }); state = normalizeState(data.state || data.data); renderAll(); toast('Recipe deleted', 'success'); } catch(e) { toast(e.message, 'error'); } };
window.reverseVardanaProduction = async (id) => { if (!requireAdmin()) return; if (!confirm('Production entry reverse karein? Finished stock minus aur vardana wapas add hoga.')) return; try { const data = await api('/api/vardana/production/' + encodeURIComponent(id) + '/reverse', { method: 'POST' }); state = normalizeState(data.state || data.data); renderAll(); toast('Production reversed', 'success'); } catch(e) { toast(e.message, 'error'); } };

function initEvents() {
  $$('.nav-btn').forEach(btn => btn.addEventListener('click', () => { $$('.nav-btn').forEach(b => b.classList.remove('active')); btn.classList.add('active'); $$('.screen').forEach(s => s.classList.remove('active')); $('screen-' + btn.dataset.screen).classList.add('active'); const titles = { dashboard: ['Warehouse dashboard','Inventory, pending orders, dispatch aur MIS ek jagah.'], inventory: ['Inventory','Stock master aur shortage planning.'], vardana: ['Vardana','Packing material stock, recipe aur production stock in.'], orders: ['Orders','Pending aur completed orders tracker.'], masters: ['Master data','Parties aur salesman login management.'], targets: ['Targets','Salesman category/product target tracker.'], reports: ['MIS report','Business report aur filters.'], settings: ['Settings','Backup, restore aur phone web app link.'] }; $('pageTitle').textContent = titles[btn.dataset.screen][0]; $('pageSubtitle').textContent = titles[btn.dataset.screen][1]; document.body.classList.remove('nav-open'); renderAll(); }));
