...
function quickProductionRowsForSelected() {
  ...
  return Object.values(map).map(x => ({ product: x.product, unit: x.unit, category: x.category, qty: x.qty, orders: x.orders.size, parties: Array.from(x.parties).filter(Boolean) })).sort((a,b)=>String(a.category).localeCompare(String(b.category)) || compareProductsBySequence(a.product, b.product));
}
...
function renderQuickProductionHistory() {
  ...
  if (!body) return;
  const rows = Array.isArray(state.quickProductions) ? state.quickProductions.slice(0, 80) : [];
  body.innerHTML = rows.map(h => {
    ...
    return `<tr><td><b>${escapeHtml(h.date || '')}</b><br><span class="muted">${escapeHtml(h.time || '')}</span></td><td>${items}</td><td>${orders}</td><td>${escapeHtml(h.note || '')}</td></tr>`;
  }).join('') || '<tr><td colspan="4" class="empty-state">Abhi quick production entry nahi hai.</td></tr>';
}
